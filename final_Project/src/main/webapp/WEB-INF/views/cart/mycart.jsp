			...
			<c:if test="${empty requestScope.cartlist}">
				<div style="text-align: center;">
					<i class="bi bi-cart-x"
						style="text-align: center; font-size: 250px; color: #e7e7e7;"></i>
					<h2 style="text-align: center; padding-bottom: 20px;">장바구니에
						상품이 존재하지 않습니다.</h2>
					<a class="a_hover" style="padding-bottom: 20px; color: #bac600;"
				...
				<form>
						...
						<table class="cart_table"  border="1" id="id_cart" name="id_cart" style="text-align: center;">
							...
							<c:forEach var="data" items="${requestScope.cartlist }">
								<tr class="cart_tr">
									<td>
										<div class="checkBox">
											<input type="checkbox" name="chBox" class="chBox"
												data-cartNum="${data.id}" checked="checked" />
										</div>
									</td>
									<td><a href="${detail}?id=${data.getPid()}"> <img
											class="rounded card-img-top productImg"
									...
									<td>
										<%-- ${data.qty } * ${data.days} =  --%>${data.qty * data.days}</td>
									<td>${data.money }</td>
								</tr>
							</c:forEach>
						</table>
					...
					<div style="float: right; padding-bottom: 200px;">
						<table class="cart_table"  border="1" id="id_price" name="id_price"
							...
							<tr class="cart_tr">
								<th class="cart_th"  style="border-top: solid 5px #e1e1e1; padding-bottom: 10px;">총 상품 금액</th>
								<td style="border-top: solid 5px #e1e1e1; padding-top: 10px; padding-bottom: 13px; font-size: 20px; font-weight: bold;">${sumMoney }</td>
							</tr>
							<tr class="cart_tr">
								<th class="cart_th"  style="border-top: solid 1px #e1e1e1; padding-bottom: 10px;">배송비<br>
								<span style="color: gray; font-size: 10px;">* 30000원 이상 주문 시 무료</span></th>
							...
							<tr class="cart_tr">
								<th class="cart_th"  style="border-top: solid 1px #e1e1e1; padding-bottom: 10px;">결제 예정 금액</th>
